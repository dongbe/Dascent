...
!function (a, b, c) {
...
}(this), Object.prototype.hasOwnProperty || (Object.prototype.hasOwnProperty = function (a) {
  var b = this.__proto__ || this.constructor.prototype;
  return a in this && (!(a in b) || b[a] !== this[a])
}), window.requestAnimationFrame || (window.webkitRequestAnimationFrame ? !function (a) {
  a.requestAnimationFrame = function (b) {
    return webkitRequestAnimationFrame(function () {
      b(a.performance.now())
    })
  }, a.cancelAnimationFrame = webkitCancelAnimationFrame
}(this) : window.mozRequestAnimationFrame ? !function (a) {
  a.requestAnimationFrame = function (b) {
  ...
  function c(a, b, c, f) {
    ...
    g && e({mode: null, el: f, flags: g, engineSettings: b})
  }

  function d(a, b) {
    for (var c = {
      theme: s(G.settings.themes.gray, null),
      ...
      var l = !1;
      if (G.flags.dimensions.match(j))d = !0, c.dimensions = G.flags.dimensions.output(j), l = !0; else if (G.flags.fluid.match(j))d = !0, c.dimensions = G.flags.fluid.output(j), c.fluid = !0, l = !0; else if (G.flags.textmode.match(j))c.textmode = G.flags.textmode.output(j), l = !0; else if (G.flags.colors.match(j)) {
        var m = G.flags.colors.output(j);
        c.theme = s(c.theme, m), l = !0
      } else if (b.themes[j])b.themes.hasOwnProperty(j) && (c.theme = s(b.themes[j], null)), l = !0; else if (G.flags.font.match(j))c.font = G.flags.font.output(j), l = !0; else if (G.flags.auto.match(j))c.auto = !0, l = !0; else if (G.flags.text.match(j))c.text = G.flags.text.output(j), l = !0; else if (G.flags.size.match(j))c.size = G.flags.size.output(j), l = !0; else if (G.flags.random.match(j)) {
        null == G.vars.cache.themeKeys && (G.vars.cache.themeKeys = Object.keys(b.themes));
        var n = G.vars.cache.themeKeys[0 | Math.random() * G.vars.cache.themeKeys.length];
        c.theme = s(b.themes[n], null), l = !0
      }
  ...
  function e(a) {
    var b = a.mode, c = a.el, d = a.flags, e = a.engineSettings, g = d.dimensions, h = d.theme, k = g.width + "x" + g.height;
    if (b = null == b ? d.fluid ? "fluid" : "image" : b, null != d.text && (h.text = d.text, "object" === c.nodeName.toLowerCase())) {
      for (var l = h.text.split("\\n"), m = 0; m < l.length; m++)l[m] = y(l[m]);
      h.text = l.join("\\n")
    }
    var n = d.holderURL, o = s(e, null);
    d.font && (h.font = d.font, !o.noFontFallback && "img" === c.nodeName.toLowerCase() && G.setup.supportsCanvas && "svg" === o.renderer && (o = s(o, {renderer: "canvas"}))), d.font && "canvas" == o.renderer && (o.reRender = !0), "background" == b ? null == c.getAttribute("data-background-src") && p(c, {"data-background-src": n}) : p(c, {"data-src": n}), d.theme = h, c.holderData = {
      flags: d,
      engineSettings: o
    }, ("image" == b || "fluid" == b) && p(c, {alt: h.text ? h.text + " [" + k + "]" : k});
    var q = {mode: b, el: c, holderSettings: {dimensions: g, theme: h, flags: d}, engineSettings: o};
    "image" == b ? ("html" != o.renderer && d.auto || (c.style.width = g.width + "px", c.style.height = g.height + "px"), "html" == o.renderer ? c.style.backgroundColor = h.background : (f(q), "exact" == d.textmode && (c.holderData.resizeUpdate = !0, G.vars.resizableImages.push(c), i(c)))) : "background" == b && "html" != o.renderer ? f(q) : "fluid" == b && (c.holderData.resizeUpdate = !0, "%" == g.height.slice(-1) ? c.style.height = g.height : null != d.auto && d.auto || (c.style.height = g.height + "px"), "%" == g.width.slice(-1) ? c.style.width = g.width : null != d.auto && d.auto || (c.style.width = g.width + "px"), ("inline" == c.style.display || "" === c.style.display || "none" == c.style.display) && (c.style.display = "block"), j(c), "html" == o.renderer ? c.style.backgroundColor = h.background : (G.vars.resizableImages.push(c), i(c)))
  }
  ...
  function f(a) {
    ...
    var d = null, e = a.mode, f = a.holderSettings, h = a.el, i = a.engineSettings;
    switch (i.renderer) {
      case"svg":
        if (!G.setup.supportsSVG)return;
        break;
      case"canvas":
        if (!G.setup.supportsCanvas)return;
        break;
      default:
        return
    }
    var j = {width: f.dimensions.width, height: f.dimensions.height, theme: f.theme, flags: f.flags}, k = g(j);
    if (d = c(), null == d)throw"Holder: couldn't render placeholder";
    "background" == e ? (h.style.backgroundImage = "url(" + d + ")", h.style.backgroundSize = j.width + "px " + j.height + "px") : ("img" === h.nodeName.toLowerCase() ? p(h, {src: d}) : "object" === h.nodeName.toLowerCase() && (p(h, {data: d}), p(h, {type: "image/svg+xml"})), i.reRender && b.setTimeout(function () {
      var a = c();
      if (null == a)throw"Holder: couldn't render placeholder";
      "img" === h.nodeName.toLowerCase() ? p(h, {src: a}) : "object" === h.nodeName.toLowerCase() && (p(h, {data: a}), p(h, {type: "image/svg+xml"}))
    }, 100)), p(h, {"data-holder-rendered": !0})
  }
  ...
  function g(a) {
    ...
    switch (parseFloat(a.theme.size) ? c = a.theme.size : parseFloat(a.flags.size) && (c = a.flags.size), a.font = {
      ...
      size: h(a.width, a.height, c),
      units: a.theme.units ? a.theme.units : G.defaults.units,
    ...
    var i = g.textPositionData = H(d);
    if (!i)throw"Holder: staging fallback not supported yet.";
    g.properties.leading = i.boundingBox.height;
    var j = null, k = null;
    if (i.lineCount > 1) {
      var l = 0, m = 0, n = a.width * G.setup.lineWrapRatio, o = 0;
      k = new e.Group("line" + o);
      for (var p = 0; p < i.words.length; p++) {
        var q = i.words[p];
        j = new e.Text(q.text);
        var r = "\\n" == q.text;
        (l + q.width >= n || r === !0) && (b(g, k, l, g.properties.leading), l = 0, m += g.properties.leading, o += 1, k = new e.Group("line" + o), k.y = m), r !== !0 && (j.moveTo(l, 0), l += i.spaceWidth + q.width, k.add(j))
      }
    ...
    } else j = new e.Text(a.text), k = new e.Group("line0"), k.add(j), g.add(k), g.moveTo((a.width - i.boundingBox.width) / 2, (a.height - i.boundingBox.height) / 2, null);
    return d
  }

  function h(a, b, c) {
    var d = parseInt(a, 10), e = parseInt(b, 10), f = Math.max(d, e), g = Math.min(d, e), h = .8 * Math.min(g, f * G.defaults.scale);
    return Math.round(Math.max(c, h))
  }

  function i(a) {
    var b;
    b = null == a || null == a.nodeType ? G.vars.resizableImages : [a];
    for (var c = 0, d = b.length; d > c; c++) {
      var e = b[c];
      if (e.holderData) {
        var g = e.holderData.flags, h = k(e);
        if (h) {
          ...
          var j = {
            mode: "image",
            holderSettings: {dimensions: h, theme: g.theme, flags: g},
            el: e,
            engineSettings: e.holderData.engineSettings
          };
          "exact" == g.textmode && (g.exactDimensions = h, j.holderSettings.dimensions = g.dimensions), f(j)
        } else n(e)
      }
    }
  }
  ...
  function l() {
    for (var a, c = [], d = Object.keys(G.vars.invisibleImages), e = 0, f = d.length; f > e; e++)a = G.vars.invisibleImages[d[e]], k(a) && "img" == a.nodeName.toLowerCase() && (c.push(a), delete G.vars.invisibleImages[d[e]]);
    c.length && F.run({images: c}), b.requestAnimationFrame(l)
  }

  function m() {
    G.vars.visibilityCheckStarted || (b.requestAnimationFrame(l), G.vars.visibilityCheckStarted = !0)
  }

  function n(a) {
    a.holderData.invisibleId || (G.vars.invisibleId += 1, G.vars.invisibleImages["i" + G.vars.invisibleId] = a, a.holderData.invisibleId = G.vars.invisibleId)
  }

  function o(a, b) {
    return null == b ? C.createElement(a) : C.createElementNS(b, a)
  }
  ...
  function q(a, b, c) {
    ...
    a.webkitMatchesSelector && a.setAttribute("xmlns", A);
    for (var e = 0; e < a.childNodes.length; e++)a.childNodes[e].nodeType === B && a.removeChild(a.childNodes[e]);
  ...
  function u(a) {
    G.vars.debounceTimer || a.call(this), G.vars.debounceTimer && b.clearTimeout(G.vars.debounceTimer), G.vars.debounceTimer = b.setTimeout(function () {
      G.vars.debounceTimer = null, a.call(this)
    }, G.setup.debounce)
  }
  ...
  function w(a) {
    var c = null;
    return "string" == typeof a ? c = C.querySelectorAll(a) : b.NodeList && a instanceof b.NodeList ? c = a : b.Node && a instanceof b.Node ? c = [a] : b.HTMLCollection && a instanceof b.HTMLCollection ? c = a : a instanceof Array ? c = a : null === a && (c = []), c
  }
  ...
  var A = "http://www.w3.org/2000/svg", B = 8, C = b.document, D = "2.5.2", E = "\nCreated with Holder.js " + D + ".\nLearn more at http://holderjs.com\n(c) 2012-2015 Ivan Malopinsky - http://imsky.co\n", F = {
    version: D,
    addTheme: function (a, b) {
      return null != a && null != b && (G.settings.themes[a] = b), delete G.vars.cache.themeKeys, this
    },
    addImage: function (a, b) {
      var c = C.querySelectorAll(b);
      if (c.length)for (var d = 0, e = c.length; e > d; d++) {
        var f = o("img");
        p(f, {"data-src": a}), c[d].appendChild(f)
      }
      return this
    },
    setResizeUpdate: function (a, b) {
      a.holderData && (a.holderData.resizeUpdate = !!b, a.holderData.resizeUpdate && i(a))
    },
    run: function (a) {
      a = a || {};
      var f = {};
      G.vars.preempted = !0;
      var g = s(G.settings, a);
      f.renderer = g.renderer ? g.renderer : G.setup.renderer, -1 === G.setup.renderers.join(",").indexOf(f.renderer) && (f.renderer = G.setup.supportsSVG ? "svg" : G.setup.supportsCanvas ? "canvas" : "html");
      var h = w(g.images), i = w(g.bgnodes), j = w(g.stylenodes), k = w(g.objects);
      ...
      for (var l = 0; l < j.length; l++) {
        ...
        if (m.attributes.rel && m.attributes.href && "stylesheet" == m.attributes.rel.value) {
          ...
          f.stylesheets.push(q)
        }
      }
      for (l = 0; l < i.length; l++)if (b.getComputedStyle) {
        var r = b.getComputedStyle(i[l], null).getPropertyValue("background-image"), t = i[l].getAttribute("data-background-src"), u = null;
        u = null == t ? r : t;
        var v = null, y = "?" + g.domain + "/";
        if (0 === u.indexOf(y))v = u.slice(1); else if (-1 != u.indexOf(y)) {
          var z = u.substr(u.indexOf(y)).slice(1), A = z.match(/([^\"]*)"?\)/);
          null != A && (v = A[1])
        }
        if (null != v) {
          var B = d(v, g);
          B && e({mode: "background", el: i[l], flags: B, engineSettings: f})
        }
      }
      for (l = 0; l < k.length; l++) {
        var C = k[l], D = {};
        try {
          D.data = C.getAttribute("data"), D.dataSrc = C.getAttribute("data-src")
        } catch (E) {
        }
        var F = null != D.data && 0 === D.data.indexOf(g.domain), H = null != D.dataSrc && 0 === D.dataSrc.indexOf(g.domain);
        F ? c(g, f, D.data, C) : H && c(g, f, D.dataSrc, C)
      }
    ...
    return function (d) {
      var e = d.root;
      if (G.setup.supportsSVG) {
        var f = !1, g = function (a) {
          return C.createTextNode(a)
        };
        (null == a || a.parentNode !== C.body) && (f = !0), a = q(a, e.properties.width, e.properties.height), a.style.display = "block", f && (b = o("text", A), c = g(null), p(b, {x: 0}), b.appendChild(c), a.appendChild(b), C.body.appendChild(a), a.style.visibility = "hidden", a.style.position = "absolute", a.style.top = "-100%", a.style.left = "-100%");
        ...
        var j = b.getBBox(), k = Math.ceil(j.width / (e.properties.width * G.setup.lineWrapRatio)), l = i.text.split(" "), m = i.text.match(/\\n/g);
        k += null == m ? 0 : m.length, c.nodeValue = i.text.replace(/[ ]+/g, "");
        var n = b.getComputedTextLength(), r = j.width - n, s = Math.round(r / Math.max(1, l.length - 1)), u = [];
        if (k > 1) {
          c.nodeValue = "";
          for (var v = 0; v < l.length; v++)if (0 !== l[v].length) {
            c.nodeValue = z(l[v]);
            var w = b.getBBox();
            u.push({text: l[v], width: w.width})
          }
        }
        return a.style.display = "none", {spaceWidth: s, lineCount: k, boundingBox: j, words: u}
      }
      return !1
    }
  }(), I = function () {
    ...
    return function (c) {
      ...
      var d = c.root;
      a.width = G.dpr(d.properties.width), a.height = G.dpr(d.properties.height), b.textBaseline = "middle", b.fillStyle = d.children.holderBg.properties.fill, b.fillRect(0, 0, G.dpr(d.children.holderBg.width), G.dpr(d.children.holderBg.height));
      {
        var e = d.children.holderTextGroup;
        e.properties
      }
      b.font = e.properties.font.weight + " " + G.dpr(e.properties.font.size) + e.properties.font.units + " " + e.properties.font.family + ", monospace", b.fillStyle = e.properties.fill;
      for (var f in e.children) {
        var g = e.children[f];
        for (var h in g.children) {
          var i = g.children[h], j = G.dpr(e.x + g.x + i.x), k = G.dpr(e.y + g.y + i.y + e.properties.leading / 2);
    ...
    if (b.XMLSerializer) {
      var a = q(null, 0, 0), c = o("rect", A);
      return a.appendChild(c), function (b, d) {
        var e = b.root, f = d.holderSettings.flags.holderURL, g = C.createComment("\nSource URL: " + f + E);
        q(a, e.properties.width, e.properties.height), a.insertBefore(g, a.firstChild);
        for (var h = a.querySelectorAll("g"), i = 0; i < h.length; i++)h[i].parentNode.removeChild(h[i]);
        p(c, {
          width: e.children.holderBg.width,
          height: e.children.holderBg.height,
          fill: e.children.holderBg.properties.fill
        });
        var j = e.children.holderTextGroup, k = j.properties, l = o("g", A), m = j.textPositionData;
        a.appendChild(l), j.y += .8 * m.boundingBox.height;
        for (var n in j.children) {
          var s = j.children[n];
          for (var u in s.children) {
            var v = s.children[u], w = j.x + s.x + v.x, x = j.y + s.y + v.y, y = o("text", A), z = C.createTextNode(null);
            p(y, {
              x: w,
              y: x,
              style: t({
                fill: k.fill,
                "font-weight": k.font.weight,
                "font-family": k.font.family + ", monospace",
                "font-size": k.font.size + k.font.units
              })
            }), z.nodeValue = v.properties.text, y.appendChild(z), l.appendChild(y)
          }
        }
        var B = "data:image/svg+xml;base64," + btoa(unescape(encodeURIComponent(r(a, d.engineSettings))));
        return B
      }
    }
  }(), K = function (a) {
  ...
  for (var L in G.flags)G.flags.hasOwnProperty(L) && (G.flags[L].match = function (a) {
  ...
  G.setup = {
    ...
    d.getContext && -1 != d.toDataURL("image/png").indexOf("data:image/png") && (G.setup.renderer = "canvas", e = d.getContext("2d"), G.setup.supportsCanvas = !0), G.setup.supportsCanvas && (a = b.devicePixelRatio || 1, c = e.webkitBackingStorePixelRatio || e.mozBackingStorePixelRatio || e.msBackingStorePixelRatio || e.oBackingStorePixelRatio || e.backingStorePixelRatio || 1), G.setup.ratio = a / c, C.createElementNS && C.createElementNS(A, "svg").createSVGRect && (G.setup.renderer = "svg", G.setup.supportsSVG = !0)
  }(), m(), a(F, "Holder", b), b.onDomReady && b.onDomReady(function () {
    G.vars.preempted || F.run(), b.addEventListener ? (b.addEventListener("resize", v, !1), b.addEventListener("orientationchange", v, !1)) : b.attachEvent("onresize", v), "object" == typeof b.Turbolinks && b.document.addEventListener("page:change", function () {
      F.run()
    })
  })
}(function (a, b, c) {
